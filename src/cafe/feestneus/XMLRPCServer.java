...
public class XMLRPCServer extends XMLRPCCommon {
	...
	public XMLRPCMethodCall readMethodCall(Socket socket) throws IOException, XmlPullParserException
	{
		XMLRPCMethodCall methodCall = new XMLRPCMethodCall();
		InputStream inputStream = socket.getInputStream();

		XmlPullParser pullParser = xmlPullParserFromSocket(inputStream);
		
		pullParser.nextTag();
		pullParser.require(XmlPullParser.START_TAG, null, XMLRPCTag.METHOD_CALL);
		pullParser.nextTag();
		pullParser.require(XmlPullParser.START_TAG, null, XMLRPCTag.METHOD_NAME);

		methodCall.setMethodName(pullParser.nextText());

		pullParser.nextTag();
		pullParser.require(XmlPullParser.START_TAG, null, XMLRPCTag.PARAMS);
		pullParser.nextTag(); // <param>
		
		do {
			//Log.d(Tag.LOG, "type=" + pullParser.getEventType() + ", tag=" + pullParser.getName());
			pullParser.require(XmlPullParser.START_TAG, null, XMLRPCTag.PARAM);
			...
			pullParser.nextTag();
			pullParser.require(XmlPullParser.END_TAG, null, XMLRPCTag.PARAM);
			pullParser.nextTag(); // <param> or </params>
			
		} while (!pullParser.getName().equals(XMLRPCTag.PARAMS)); // </params>
	...
	public void respond(Socket socket, Object[] params) throws IOException {
		...
		outputStream.close();
		socket.close();
		Log.d(XMLRPCTag.LOG, "response:" + response);
	}
	...
	throws IllegalArgumentException, IllegalStateException, IOException {
		...
		serializer.startTag(null, XMLRPCTag.METHOD_RESPONSE);
		
		serializeParams(params);

		serializer.endTag(null, XMLRPCTag.METHOD_RESPONSE);
