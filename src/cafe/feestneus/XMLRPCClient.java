...
// reader = new StringReader("<?xml version='1.0'?><methodResponse><params><param><value>\n\n\n</value></param></params></methodResponse>");
			...
			// lets start pulling...
			pullParser.nextTag();
			pullParser.require(XmlPullParser.START_TAG, null, XMLRPCTag.METHOD_RESPONSE);
			
			pullParser.nextTag(); // either Tag.PARAMS (<params>) or Tag.FAULT (<fault>)  
			String tag = pullParser.getName();
			if (tag.equals(XMLRPCTag.PARAMS)) {
				// normal response
			...
			if (tag.equals(XMLRPCTag.FAULT)) {
				...
				String faultString = (String) map.get(XMLRPCTag.FAULT_STRING);
				int faultCode = (Integer) map.get(XMLRPCTag.FAULT_CODE);
				entity.consumeContent();
	...
	throws IllegalArgumentException, IllegalStateException, IOException {
		...
		serializer.startTag(null, XMLRPCTag.METHOD_CALL);
		// set method name
		serializer.startTag(null, XMLRPCTag.METHOD_NAME).text(method).endTag(null, XMLRPCTag.METHOD_NAME);
		
		serializeParams(params);

		serializer.endTag(null, XMLRPCTag.METHOD_CALL);
